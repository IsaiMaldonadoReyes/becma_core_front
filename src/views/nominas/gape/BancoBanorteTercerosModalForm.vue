  ...
  <v-dialog v-model="dialogPropiedades.dialog" :width="smAndDown ? '95vw' : '35vw'" persistent>
    <v-card>
      <v-form ref="formRef">
        ...
        <v-card-text class="dialog-content px-5 mb-4" :style="getDialogContentPaddingTop">
          <v-row>
            <v-col cols="12" md="12">
              <bec-text-field
                v-model="dialogPropiedades.elementos.cuenta_origen"
                :label="'Cuenta origen'"
                ...
                <template #tooltip>
                  <v-card color="transparent" elevation="0" class="pa-3">
                    <v-row>
                      ...
                      <v-col cols="11">
                        Capture la Cuenta origen correspondiente para generar el layout de
                        dispersión del Banco Banorte terceros.
                      </v-col>
                    </v-row>
                  </v-card>
                </template>
            ...
            <v-col cols="12" md="12">
              <bec-text-field
                v-model="dialogPropiedades.elementos.clave_banco"
                :label="'Clave banco'"
                ...
                <template #tooltip>
                  <v-card color="transparent" elevation="0" class="pa-3">
                    <v-row>
                      ...
                      <v-col cols="11">
                        Capture la Clave banco correspondiente para generar el layout de dispersión
                        del Banco Banorte terceros.
                      </v-col>
                    </v-row>
                  </v-card>
                </template>
...
export default defineComponent({
  name: 'BancoBanorteTercerosModalForm',
  components: { BecSelect, BecAutocomplete, BecTextField },
  props: {
    dialogEvent: String,
    dialogItems: { type: Object as () => Partial<BanorteTerceros>, required: true },
    dialogTitle: String,
    dialogView: Boolean,
  },
  setup(props, { emit }) {
    ...
    const onDecision = () => {
      ...
      if (dialogPropiedades.value.elementos.id) {
        ...
        mensaje = `¿Desea actualizar el registro?`
      } else {
