<template>
                          ...
                          >
                            <v-card-text class="text-grey-darken-1">
                              <vue3-lottie
                                width="50vw"
                                height="35vh"
                                :animation-link="'/src/assets/images/no_data_kiosko.json'"
                              />
                            </v-card-text>
                          </v-card>
                        </template>
  ...
  <div class="text-center">
    <v-overlay :model-value="loadingEsperaPeticion" class="align-center justify-center">
      <v-progress-circular color="#722B81" size="64" indeterminate></v-progress-circular>
    </v-overlay>
  </div>
</template>
<script lang="ts">
...
export default defineComponent({
  ...
  setup() {
    ...
    watch(
      () => modelKiosco.value.rfc,
      async (nuevoRFC) => {
        // 1️⃣ Normaliza el texto
        const rfc = nuevoRFC?.trim().toUpperCase()

        // 2️⃣ Valida longitud RFC (12 o 13 caracteres)
        if (rfc && (rfc.length === 12 || rfc.length === 13)) {
          try {
            await storeKiosko.storeBuscarCliente({
              id_sucursal: modelKiosco.value.id_sucursal,
              rfc: rfc,
            })

            const data = storeKiosko.resultCliente

            if (data && data.code === 200 && data.data) {
              const cliente = data.data

              modelKiosco.value.razon_social = cliente.CRAZONSOCIAL || ''
              modelKiosco.value.correo = cliente.CEMAIL1 || ''
              modelKiosco.value.codigo_postal = cliente.CCODIGOPOSTAL || ''

              if (cliente.CCODIGOPOSTAL) {
                await buscarDireccionPorCodigo(cliente.CCODIGOPOSTAL)
              }

              //modelKiosco.value.colonia = cliente.CCOLONIA || ''
              modelKiosco.value.numero_exterior = cliente.CNUMEROEXTERIOR || ''
              modelKiosco.value.numero_interior = cliente.CNUMEROINTERIOR || ''
              modelKiosco.value.calle = cliente.CNOMBRECALLE || ''
              modelKiosco.value.uso_cfdi = cliente.CUSOCFDI || ''
              modelKiosco.value.regimen_fiscal = cliente.CREGIMFISC || ''
              // puedes llenar más campos si aplica
            } else {
              console.warn('Cliente no encontrado para RFC:', rfc)
            }
          } catch (error) {
    ...
    const buscarDireccionPorCodigo = async (codigo: string) => {
      // Limpiamos antes de comenzar la búsqueda
      itemsAsentamientos.value = []
      modelKiosco.value.colonia = null
      ...
      try {
        ...
        if (direccion && Object.keys(direccion).length > 0) {
          ...
          if (Array.isArray(colonias) && colonias.length > 0) {
            ...
            modelKiosco.value.colonia = colonias[0].d_asenta || null
          } else {
            itemsAsentamientos.value = []
            modelKiosco.value.colonia = null
          }
        }
        // 🚫 Caso: código postal no encontrado
        else {
          itemsAsentamientos.value = []
          modelKiosco.value.colonia = null
          modelKiosco.value.estado = ''
        ...
        // ⚠️ Error en la petición o en el backend
        itemsAsentamientos.value = []
        modelKiosco.value.colonia = null
        modelKiosco.value.estado = ''
        modelKiosco.value.ciudad = ''
    ...
    async function eliminarTicket() {
      try {
        await storeKiosko.storeEliminarTicket(idReciboEncabezado.value)
      } catch (error) {}
    }

    async function descargarPdf() {
      try {
        await storeKiosko.storeDescargarPdf({
          idReciboEncabezado: idReciboEncabezado.value,
        })

        const response = storeKiosko.resultEstatusTicket

        if (response.code === 200 && response.base64) {
          const link = document.createElement('a')
          link.href = `data:application/pdf;base64,${response.base64}`
          link.download = response.fileName || 'Factura.pdf'
          ...
          dialogConfirmation.onOpenDialogInformation(
            'No fue posible obtener el archivo PDF. Contacte con el administrador.',
            'Descarga fallida',
            'incorrect',
            '#B00000',
            2,
        ...
        dialogConfirmation.onOpenDialogInformation(
          'Ocurrió un error inesperado al intentar descargar el archivo PDF.',
          'Error',
          'incorrect',
          '#B00000',
          2,
    ...
    async function descargarXml() {
      try {
        await storeKiosko.storeDescargarXml({
          idReciboEncabezado: idReciboEncabezado.value,
        })
        ...
        if (response.code === 200 && response.base64) {
          const link = document.createElement('a')
          link.href = `data:application/xml;base64,${response.base64}`
          link.download = response.fileName || 'Factura.xml'
          link.style.display = 'none'
          ...
          dialogConfirmation.onOpenDialogInformation(
            'No fue posible obtener el archivo XML. Contacte con el administrador.',
            'Descarga fallida',
            'incorrect',
            '#B00000',
            2,
    ...
    const onSaveModalForm = (recibo: Recibo) => {
      ...
      if (recibo.codigo == 1) {
        ...
        if (itemsRecibos.value.length === 0) {
          ...
          idReciboEncabezado.value = recibo.idReciboEncabezado
          current.value = 2
        } else {
          dialogConfirmation.onOpenDialogInformation(
