<template>
  <v-app>
    ...
    <v-app-bar app clipped-left height="60">
      ...
      <v-spacer />
      <v-btn @click="changePrimaryColor('red')">Cambiar a Rojo</v-btn>
...
export default defineComponent({
  ...
  setup() {
    ...
    const changePrimaryColor = (color: string) => {
      // Usar `setTheme` para actualizar el tema
      theme.global.name.value = 'light' // Asegúrate de que el tema sea 'light' o 'dark'
      theme.global.setTheme({
        colors: {
          primary: color,
        },
      })
