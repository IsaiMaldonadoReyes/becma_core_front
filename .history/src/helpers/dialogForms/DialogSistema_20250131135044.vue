...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const validarFormulario = async () => {
      const { valid: esValido } = await form.value.validate()
      valid.value = esValido
    ...
    const onSave = async () => {
      ...
      if (valid) emit('save', nombreEvento.value)
    }
    ...
    const nameRules = [(value: string) => !!value || 'Este dato es requerido para continuar.']
