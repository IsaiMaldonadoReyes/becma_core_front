<template>
  <v-dialog v-model="dialogPropiedades.dialog" width="auto" persistent>
    <v-card>
      <v-card-title class="position-fixed bg-white header" ref="header2">
        <v-row>
...
<script lang="ts">
import { ref, mergeProps, watch, defineComponent, computed, nextTick, onMounted } from 'vue'
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const updateDimensions = () => {
      ...
      if (header2.value) {
        headerHeight.value = header2.value.offsetHeight
        console.log('form.value:', form.value.offsetHeight)
        console.log('header.value:', header2.value)
      }
    }
