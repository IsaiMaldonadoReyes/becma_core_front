<template>
  <v-dialog v-model="dialogPropiedades.dialog" width="auto" persistent>
    <v-card>
      <v-card-title class="position-fixed bg-white header" ref="header">
        <v-row>
      ...
      <v-card-text :style="contentStyle" class="content">
        <p v-for="i in 50" :key="i">Línea de contenido {{ i }}</p>
      </v-card-text>
...
<script lang="ts">
import { ref, mergeProps, watch, defineComponent, computed, watchEffect, nextTick } from 'vue'
  ...
  setup(props, { emit }) {
    ...
    watch(
      ...
      async (newDialogView) => {
        ...
        await nextTick()
        // Recalcular la altura del encabezado
        if (header.value) {
    ...
    // Observador para recalcular la altura del encabezado cuando cambie
    // Observar cambios en el estado del diálogo
