  ...
  <v-dialog v-model="dialogPropiedades.dialog" width="auto" persistent>
    <v-card>
      <v-form ref="form" v-model="isValid">
        ...
        <v-card-text class="content px-5" :style="contentStyle">
          <v-row>
            <v-col cols="12" md="6">
              <v-text-field
...
<script lang="ts">
import { ref, mergeProps, watch, defineComponent, computed, onMounted, onUnmounted ,nextTick} from 'vue'
...
export default defineComponent({
  ...
  if (header2.value) {
    updateDimensions(); // Calcular la altura inicial
  } else {
    console.error('El elemento header2 no está definido en onMounted.'); // Depuración
  }
  ...
  window.addEventListener('resize', updateDimensions);
    ...
    const contentStyle = computed(() => {
      /*if (header2.value) {
        const headerHeight = header2.value.$el.clientHeight + 30
        return { paddingTop: `${headerHeight}px !important` }
      }*/
      console.log('Padding-top calculado:', headerHeight.value) // Depuración
