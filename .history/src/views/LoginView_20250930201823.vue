      ...
      <v-col v-if="smAndDown" cols="12" md="2" ref="vtimContenidoRef">
        ...
        >
          <v-timeline-item
            ...
            class="cursor-pointer ma-0 pa-0 flex-grow-1"
            fill-dot
            size="60px"
            @click="goTo(i)"
            :style="getTimelineItemWidth"
          >
            ...
            <v-card
              class="d-flex text-center pa-1  flex-grow-1"
              height="80px"
              color="#E6E6E6"
              elevation="16"
...
export default defineComponent({
  ...
  setup() {
    ...
    const getTimelineItemWidth = computed(() => {
      if (vcarContenidoRef.value) {
        const ancho = (vcarContenidoRef.value.$el.clientWidth - 160) / 3;
        return { width: `${ancho}px !important` };
      }
      return { width: "0px" };
    });
