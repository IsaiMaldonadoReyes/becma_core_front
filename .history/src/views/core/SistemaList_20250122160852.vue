        ...
        <v-card width="100%" variant="elevated" elevation="10" rounded="lg">
          ...
          >
            ...
            <template v-slot:item.actions="{ item }">
              <v-icon class="me-2" icon="mdi-pencil" size="small" @click="onOpenDialogSistema("onEdit", item, "Editar sistema")" />
              <v-divider
                ...
                vertical
              ></v-divider>
              <v-icon size="small" @click="onOpenDialogAction('onEdit')"> mdi-delete </v-icon>
            </template>
...
export default defineComponent({
  ...
  setup() {
    ...
    // DialogSistema
    type MethodKeys = 'onSave' | 'onEdit' | 'onUpdate'
    ...
    function onOpenDialogSistema(evento: string, items: object, titulo: string) {
    ...
    const onSaveSistema = (evento: MethodKeys) => {
      console.log(evento)
      methods[evento]() // Ahora TypeScript sabe que 'evento' es una clave válida
    }
