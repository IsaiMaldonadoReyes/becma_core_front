...
export default defineComponent({
  ...
  setup() {
    ...
    const methods: Record<Eventos, (items: Record<string, any>) => void> = {
      onSave: () => {
        alert('Save new')
      },
      onEdit: () => {
      ...
      onDelete: (items: Elementos) => {
        console.log('Código:', items.nombre)
        dialogConfirmation.value.dialog = false
        onOpenDialogInformation(
          '#438701',
          `Esta acción eliminará ${items} de forma definitiva. ¿Desea continuar?`,
          'onEdit',
          'correct',
          'Registro eliminado',
          1,
