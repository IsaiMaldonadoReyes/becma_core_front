    ...
    <v-row class="py-3">
      <v-col>
        <v-card width="100%" variant="elevated" elevation="10" rounded="lg">
          <v-data-table
            v-model="vdtPrincipalItemsSeleccionados"
            v-model:search="vdtPrincipalBusqueda"
            :headers="headers"
            :items="vdtPrincipaItems"
            :mobile="smAndDown"
            :multi-sort="true"
            :sticky="true"
            item-value="codigo"
            ...
            class="pa-5"
            :hover="true"
            :items-per-page="vdtPrincipalItemsPorPagina"
          >
            <template v-slot:top>
              <v-row class="py-5">
                <v-col>
                  <v-text-field
                    v-model="vdtPrincipalBusqueda"
                    density="compact"
                    label="Buscar"
                ...
                <v-col class="d-flex justify-end">
                  <v-btn-group variant="outlined" divided rounded="xl" density="comfortable">
                      ...
                      <v-tooltip>
                        <template v-slot:activator="{ props: tooltip }">
                          <v-text-field
                            ...
                            :model-value="vdtPrincipalItemsPorPagina"
                            density="compact"
                            hide-details
                            max="15"
                            min="1"
                            ...
                            type="number"
                            variant="plain"
                            @update:model-value="vdtPrincipalItemsPorPagina = parseInt($event, 10)"
                            class="mb-1 pb-1"
                            width="50"
                          ></v-text-field>
                        </template>
                    ...
                    <v-menu>
                      ...
                      <v-list color="primary" v-model:selected="vdtPrincipalItemsSeleccionados">
                        <v-list-item
                          v-for="(item, index) in vdtPrincipalOpcionesItemsPorPagina"
                          :key="index"
                          :value="item.valor"
                        >
            ...
            <template v-slot:bottom>
              <v-pagination
                v-model="vdtPrincipalPaginaActual"
...
export default defineComponent({
  ...
  setup() {
    ...
    // Tabla
    const vdtPrincipalItemsSeleccionados = ref([])
    const vdtPrincipalBusqueda = ref('')
    const vdtPrincipalPaginaActual = ref(1)
    const vdtPrincipalItemsPorPagina = ref(5)
    const vdtPrincipalOpcionesItemsPorPagina = ref([
      { titulo: '5', valor: 5 },
      { titulo: '10', valor: 10 },
      { titulo: '15', valor: 15 },
      { titulo: '20', valor: 20 },
    ...
    const getVdtPrincipalTotalPaginas = computed(() =>
      Math.ceil(vdtPrincipaItems.value.length / vdtPrincipalItemsPorPagina.value),
    )
    ...
    return {
      ...
      headers,
      vdtPrincipalItemsPorPagina,
      vdtPrincipaItems,
      mergeProps,
      vdtPrincipalOpcionesItemsPorPagina,
      opcionesMenu,
      vdtPrincipalItemsSeleccionados,
      smAndDown,
      vdtPrincipalBusqueda,
      vdtPrincipalPaginaActual,
    }
  },
})
</script>
