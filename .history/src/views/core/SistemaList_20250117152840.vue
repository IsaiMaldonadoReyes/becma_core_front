<template>
  <v-container>
    <v-row>
      <v-card title="Sistema" subtitle="Subtitle" variant="text">
        <template v-slot:prepend>
          <v-icon size="40" icon="mdi-laptop" color="primary"></v-icon>
        </template>
      </v-card>
    </v-row>
    ...
    <v-row>
      ...
      >
        <template v-slot:header.data-table-select="{ allSelected, selectAll, someSelected }">
          <v-btn-group variant="outlined" divided rounded="xl">
            <v-btn color="black" density="compact" height="auto">
              <v-tooltip>
...
export default defineComponent({
  ...
  setup() {
    ...
    return { items, selected, headers, mergeProps, opcionesMenu, smAndDown }
  },
})
</script>
