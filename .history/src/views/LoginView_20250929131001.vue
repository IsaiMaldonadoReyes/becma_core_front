            ...
            >
              <v-tabs-window v-model="current" color="transparent">
                ...
                <v-tabs-window-item key="s1" value="s1" color="transparent">
                  ...
                  <!-- vdtiRecibosRef -->
                  <v-data-iterator :items="games" :search="search" ref="vdtiRecibosRef">
                    <template v-slot:default="{ items }">
                      <v-container class="overflow-auto" fluid :style="getRowTablaRecibosHeight">
                        <v-row>
...
export default defineComponent({
  ...
  setup() {
    ...
    const getRowTablaRecibosHeight = computed(() => {
      const alto = ref(0)
      if (vrowTituloRef.value) {
        alto.value =
          vcarContenidoRef.value.$el.clientHeight -
          vrowTituloRef.value.$el.clientHeight -
          vrowSucursalRef.value.$el.clientHeight
      }
      console.log('Car:' + vcarContenidoRef.value.$el.clientHeight)
      console.log('Titulo:' + vrowTituloRef.value.$el.clientHeight)
      console.log('Suc:' + vrowSucursalRef.value.$el.clientHeight)
