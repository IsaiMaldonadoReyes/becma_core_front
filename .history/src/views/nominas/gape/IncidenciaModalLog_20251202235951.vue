                      ...
                      >
                        <v-row class="pa-2">
                          <v-col cols="1" class="d-flex align-center justify-end">
                            ...
                            <v-divider vertical class="ml-3 my-2 border-opacity-25" />
                          </v-col>
                          <v-col cols="11">
                            <v-row>
                              <v-col cols="12" class="ma-2">
                                <v-row>
                                  <span style="font-weight: bold; color: #078929; font-size: 20px">
                                    Fila 16: Error
                                  </span>
                                </v-row>
                                <v-row>
                                  <span>
                                    El RFC, tiene un formato no válido para la información
                                  </span>
                                </v-row>
                              </v-col>
                            </v-row>
                              ...
                              >
                                ...
                                >
                                  <div class="excel-cell-wrapper">
                                    <!-- ⚠️ Ícono Excel-style solo si hay error -->
                                    <v-tooltip
                                      v-if="item.validaciones?.[header.key]"
                                      location="top"
                                    >
                                    ...
                                    <div
                                      :class="[
                                        'excel-cell',
                                        item.validaciones?.[header.key] ? 'error-cell' : '',
                                      ]"
                                    >
                                      {{ item[header.key] }}
                                    </div>
    ...
    const itemsErrores = [
      {
        ...
        salario: 350.0,
        validaciones: {
          rfc: 'El RFC no cumple con la longitud de 13 caracteres',
          salario: null,
          nombre: null,
        },
      },
...
/* 📌 Bordes en TODA la tabla: headers y celdas */
:deep(.excel-table td),
:deep(.excel-table th) {
...
/* 📌 Headers con estilo tipo Excel */
:deep(.excel-table th) {
  font-weight: bold !important;
  background-color: #f3f3f3 !important; /* <--- SIN transparencia */
  color: #333 !important;
}

/* 📌 Para que la tabla tenga scroll horizontal estilo Excel */
:deep(.excel-table table) {
  border-collapse: collapse;
  min-width: 1500px; /* Ajusta según columnas */
}
...
/* ⚠️ Icono de advertencia tipo Excel */
.error-icon {
  cursor: pointer;
}

/* 📌 Contenedor de celda + icono */
.excel-cell-wrapper {
  display: flex;
  align-items: center;
  gap: 4px;
