                    ...
                    <v-col cols="12" lg="12">
                      ...
                      >
                        <template v-slot:item.seleccionado="{ item }">
                          <v-chip :color="esSeleccionado(item) ? 'green' : 'red'">
                            {{ esSeleccionado(item) ? "Sí" : "No" }}
                          </v-chip>
                        </template>
...
export default defineComponent({
  ...
  setup() {
    ...
    const seleccionadosSet = computed(
      () => new Set(itemsSeleccionados.map((i) => i.concepto))
    );

    const esSeleccionado = (item: any) => seleccionadosSet.value.has(item.concepto);
