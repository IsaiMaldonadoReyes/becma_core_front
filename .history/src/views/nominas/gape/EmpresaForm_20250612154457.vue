                  ...
                  <v-row>
                      ...
                      >
                        ...
                        <template v-slot:item.tope="{ item }">
                          <v-text-field
                            ...
                            hide-details
                            prefix="$"
                            @blur="() => formatearCantidad(item)"
                            @focus="() => mostrarCrudo(item)"
                            :placeholder="'0.00'"
                          />
                        </template>
                    ...
                    <v-col cols="12" lg="12">
                      <bec-select
                        ...
                        :label="'Campo 1'"
                        :multiple="false"
                        :onModelUpdate="myCustomUpdate"
                        :placeholder="'Seleccione'"
                        :prepend-icon="'mdi-account'"
                        :rules="[(v) => !!v || 'Este campo es requerido']"
                        :tooltip="'<b>Hola</b> sss'"
                      />
                      <bec-autocomplete
                        v-model="modelEmpresa2"
                        :disabled="false"
                        :itemTitle="'nombre_empresa'"
                        :itemValue="'id'"
                        ...
                        :return-object="false"
                        :rules="[(v) => !!v || 'Este campo es requerido']"
                        :tooltip="'Seleccione <b>por favor</b>'"
                      />
                      <bec-text-field
                        v-model="modelClasePrimaRiesgo"
                        :label="'Prueba componente text Field'"
                        :onModelUpdate="myCustomUpdate"
                        :placeholder="''"
                        :prepend-icon="'mdi-account'"
                        :rules="[(v) => !!v || 'Este campo es requerido']"
                    ...
                    <v-col
                      v-for="(campo, index) in camposTab1"
                      :key="index"
                      cols="12"
                      lg="12"
                    >
...
export default defineComponent({
  ...
  setup() {
    ...
    const headers = [
      { title: "Concepto", key: "concepto", width: "10%", sortable: false },
      { title: "Tope", key: "tope", width: "10%", sortable: false },
    ...
    const formattedValue = computed({
      get: () => {
        if (rawValue.value == null || isNaN(rawValue.value)) return "";
        return rawValue.value
          ...
          .replace("$", ""); // usamos prefix="$"
      },
      set: (val: string) => {
        const clean = parseFloat(
          val.replace(/[^\d.-]/g, "") // eliminar $, comas, etc.
        );
        rawValue.value = isNaN(clean) ? null : clean;
      },
    });

    // Opcional: volver a formatear al salir del campo
    function formatOnBlur() {
      formattedValue.value = formattedValue.value; // fuerza reevaluación
    }

    // Opcional: mostrar número limpio al enfocar
    function rawOnFocus(event: Event) {
      const input = event.target as HTMLInputElement;
      if (rawValue.value !== null) {
        input.value = rawValue.value.toString();
      }
    }
