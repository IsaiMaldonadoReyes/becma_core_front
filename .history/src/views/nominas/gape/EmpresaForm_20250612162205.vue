...
export default defineComponent({
  ...
  setup() {
    ...
    function formatearCantidad(item: any) {
      if (!item || isNaN(item.tope)) return;
      const value = parseFloat(item.tope);
      item.tope = value
    ...
    function mostrarCrudo(item: any) {
      if (!item || typeof item.tope !== "string") return;
      const clean = parseFloat(item.tope.replace(/[^\d.-]/g, ""));
      item.tope = isNaN(clean) ? "" : clean.toString();
    }

    function rowProps(item: any, index: number) {
      return {
        class: item.seleccionado ? "bg-blue-lighten-5" : "",
      };
    }
