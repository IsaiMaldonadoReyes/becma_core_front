        ...
        <v-card-text class="dialog-content px-5" :style="getDialogContentPaddingTop">
          <v-data-iterator :items="itemsErrores" items-per-page="-1">
            <template v-slot:default="{ items }">
              <v-card
                v-for="grupo in items"
                :key="grupo.raw.agrupador"
                class="rounded pa-3 ma-5"
                color="#f0f0f0"
                elevation="3"
              >
                <v-row>
                ...
                <!-- 🔥 Segundo nivel: errores dentro del agrupador -->
                <v-row v-for="err in grupo.raw.errores" :key="err.tipo">
                  ...
                  <v-col cols="5">
                    <span style="font-weight: bold; color: #af1023; font-size: 18px"> Error: </span>
                    <br />
                    <span> Tipo de error: {{ err.tipo }} </span>
                  </v-col>
