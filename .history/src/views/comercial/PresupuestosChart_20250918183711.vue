<template>
  <v-container ref="vconPrincipalRef" fluid style="height: 95% !important">
    ...
    <presupuestos-modal-chart
      :dialog-event="dialogPresupuestosChartPropiedades.evento"
      :dialog-items="dialogPresupuestosChartPropiedades.items"
...
// import views
import PresupuestoModalChart from '@/views/comercial/PresupuestosModalChart.vue'
...
export default defineComponent({
  ...
  components: { Vue3Lottie, BecAutocomplete, BecSelect, PresupuestoModalChart },
  ...
  setup() {
    ...
    const optionsChartBarV: ChartOptions = {
      ...
      onClick: (e, elements, chart) => {
        if (elements.length > 0) {
          ...
          onOpenDialogPresupuestosChart('onEdit', [], 'Editar cliente')
        }
      },
    }
    ...
    const onOpenDialogPresupuestosChart = (evento: string, items: object, titulo: string) => {
      dialogPresupuestosChartPropiedades.value = {
        dialog: true,
        evento: evento,
        items: items,
        titulo: titulo,
