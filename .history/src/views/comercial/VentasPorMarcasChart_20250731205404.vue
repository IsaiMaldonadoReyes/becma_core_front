  ...
  setup() {
    ...
    async function exportChartToPdf(canvas: HTMLCanvasElement, filename = 'grafica.pdf') {
      ...
      const pdf = new jsPDF({
        ...
        unit: 'pt',
        format: 'letter', // tamaño carta: 612 x 792 pt
      })

      const pageWidth = pdf.internal.pageSize.getWidth()
      const pageHeight = pdf.internal.pageSize.getHeight()

      // Margen deseado en puntos (por ejemplo 40pt)
      const margin = 40

      // Espacio útil dentro de la hoja descontando los márgenes
      const usableWidth = pageWidth - margin * 2
      const usableHeight = pageHeight - margin * 2

      // Escalar imagen proporcionalmente para que quepa dentro del área útil
      const ratio = Math.min(usableWidth / canvasImage.width, usableHeight / canvasImage.height)
      const imgWidth = canvasImage.width * ratio
      const imgHeight = canvasImage.height * ratio

      // Posicionar imagen centrada dentro del área útil
