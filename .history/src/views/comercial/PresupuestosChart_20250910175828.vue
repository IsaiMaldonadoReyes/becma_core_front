...
export default defineComponent({
  ...
  setup() {
    ...
    const modelMarca = ref<string | null>(null)
    const itemsMarcas = ref<Marca[]>([])

    const modelAgente = ref<string | null>(null)
    const itemsAgentes = ref<Agente[]>([])

    const modelPeriodoInicial = ref<{ label: string; value: number } | null>(null)

    const modelPeriodoFinal = ref<{ label: string; value: number } | null>(null)
    ...
    const validateRangoPeriodos = (): true | string => {
      const inicio = modelPeriodoInicial.value?.value
      const fin = modelPeriodoFinal.value?.value

      // Hasta que ambos tengan valor, no marcamos error
      if (inicio == null || fin == null) return true

      dialogStore.onOpenDialogInformation(inicio.toString(), 'Registros eliminados', 'correct', '#438701', 1)

      //return inicio < fin || 'El período inicial debe ser menor que el período final'
    }
