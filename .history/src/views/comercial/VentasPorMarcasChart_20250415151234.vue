<template>
  <v-container fluid ref="vconPrincipalRef" class="h-100">
    <v-navigation-drawer v-model="vnavFiltrosIsOpen" :width="350" location="right">
      <v-card class="overflow-auto" elevation="0" height="calc(100% - 20px)">
        <v-form ref="vforFiltrosRef">
          <v-card-text>
            <v-autocomplete
              v-model="filtroModels.empresa"
              :disabled="false"
              :items="filtroItems.empresa"
            ...
            <v-locale-provider locale="es-MX">
              <v-date-input
                v-model="filtroModels.fechas"
                :disabled="!filtroModels.empresa ? true : false"
                :mobile="smAndDown"
                :rules="[vforFiltrosRule.required]"
                clear-icon="mdi-close"
                clearable
            ...
            <v-autocomplete
              v-model="filtroModels.marcas"
              :disabled="!filtroModels.fechas ? true : false"
              :items="getMarcas"
              chips
              clear-icon="mdi-close"
              clearable
            ...
            >
              ...
              <template v-slot:item="{ props, item }">
                <v-list-item
                  v-bind="props"
                  :subtitle="item.raw.cvalorclasificacion"
            ...
            <v-list
              v-model:selected="filtroModels.modo"
            ...
            >
              ...
              <v-list-item
                v-for="item in filtroItems.modo"
                :key="item.value"
    ...
    <v-row ref="vtabGraficaRef">
      <v-col class="my-0 py-0">
        <v-tabs v-model="vtabGraficaSelected" stacked color="primary" class="text-medium-emphasis">
          <v-tab v-for="tab in vtabGraficaItems" :key="tab.value" :value="tab.value">
    ...
    <v-row>
      <v-col>
        <v-card elevation="5" class="pa-5 overflow-auto" :style="getCardHeight">
          <!--canvas ref="chartBarrasV" :style="getChartHeight"></canvas>
          <canvas ref="chartBarrasH" :style="getChartHeight"></canvas-->
...
interface Marca {
  cidvalorclasificacion: number | null
  cvalorclasificacion: string | null
}
...
export default defineComponent({
  ...
  setup() {
    ...
    const modelMarca = ref<Marca | null>(null)
    ...
    const getMarcas = computed(() => filtroItems.value.marca || [])
    ...
    const fetchChartData = async () => {
      ...
      if (isValidForm.valid) {
        ...
        const data = {
          empresa: filtroModels.value.empresa,
          marcas: filtroModels.value.marcas?.join(', '),
          modo: filtroModels.value.modo,
    ...
    const fetchEmpresas = async () => {
      try {
        ...
        filtroItems.value.empresa = reporte.empresas.data
      } catch (error) {
      } finally {
      }
    }
    ...
    const fetchMarcas = async () => {
      if (filtroModels.value) {
        ...
        try {
          ...
          filtroItems.value.marca = reporte.object.data
        } catch (error) {
        } finally {
        }
      } else {
        filtroItems.value.marca = []
        //;(filtroModels.value as FiltroModel).marcas = []
      }
    }
