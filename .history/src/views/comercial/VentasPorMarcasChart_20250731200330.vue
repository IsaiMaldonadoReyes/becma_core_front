...
export default defineComponent({
  ...
  setup() {
    ...
    async function exportChartToPdf(canvasRef: Ref<HTMLCanvasElement | null>) {
      await nextTick()

      const canvas = canvasRef.value

      if (!canvas || !document.body.contains(canvas)) {
        console.warn('Canvas no montado aún')
        return
      }
      ...
      pdf.addImage(imageData, 'PNG', 0, 0, canvasImage.width, canvasImage.height)
    ...
    async function exportCurrentChart(tabValue: string) {
      await nextTick() // Espera que se monte en el DOM

      const canvas = chartMap[tabValue]?.value

      if (!canvas) {
        console.warn('Canvas no disponible para este tab:', tabValue)
        return
      }
