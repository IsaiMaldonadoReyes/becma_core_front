    ...
    <v-card class="rounded-lg">
      <v-form ref="formRef">
        ...
        <v-card-text class="dialog-content px-5" :style="getDialogContentPaddingTop">
          <v-row>
            <v-col cols="12" md="12">
              <div v-if="dialogPresupuestosChartPropiedades.dialog" class="position-relative">
                <canvas ref="canvasChartLine2" style="height: 300px" />
              </div>
            </v-col>
          </v-row>
        </v-card-text>
  ...
  setup(props, { emit }) {
    ...
    const fetchChartData = async () => {
      ...
      if (canvasChartLine2.value) {
        instanceChartLine = new Chart(canvasChartLine2.value, {
          type: 'line',
