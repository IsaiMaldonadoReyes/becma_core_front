          ...
          <v-card-text>
            <v-row>
              ...
              <v-col cols="12">
                <bec-autocomplete
                  v-model="modelAgente"
                  :disabled="!modelMarca ? true : false"
...
export default defineComponent({
  ...
  setup() {
    ...
    const modelPeriodoInicial = ref<{ label: string; code: string; value: number } | null>(null)

    const modelPeriodoFinal = ref<{ label: string; code: string; value: number } | null>(null)

    const itemsPeriodos = [
      { code: '01', label: 'Enero', value: 1 },
      { code: '02', label: 'Febrero', value: 2 },
      { code: '03', label: 'Marzo', value: 3 },
      { code: '04', label: 'Abril', value: 4 },
      { code: '05', label: 'Mayo', value: 5 },
      { code: '06', label: 'Junio', value: 6 },
      { code: '07', label: 'Julio', value: 7 },
      { code: '08', label: 'Agosto', value: 8 },
      { code: '09', label: 'Septiembre', value: 9 },
      { code: '10', label: 'Octubre', value: 10 },
      { code: '11', label: 'Noviembre', value: 11 },
      { code: '12', label: 'Diciembre', value: 12 },
    ]
    ...
    const fetchChartData = async () => {
      ...
      if (isValidForm.valid) {
        ...
        const periodoInicial = modelEjercicio.value?.cejercicio + "-" + modelPeriodoInicial.value?.code
        ...
        const data = {
          ...
          periodoInicial: fechaInicioFormateada,
          periodoFinal: fechaFinFormateada,
        }
    ...
    const getRangoMeses = (inicio: number, fin: number) => {
      return itemsPeriodos.filter((item) => item.value >= inicio && item.value <= fin)
    }
