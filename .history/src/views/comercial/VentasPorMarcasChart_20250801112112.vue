        ...
        <v-tooltip bottom color="primary">
          <template v-slot:activator="{ props }">
            <v-btn
              ...
              class="mr-1"
              v-bind="props"
              @click="exportCurrentChart(vtabMenuModel)"
            >
...
import { type Chart, Chart, type ChartData, type ChartOptions } from 'chart.js/auto'
...
export default defineComponent({
  ...
  setup() {
    ...
    const canvasChartBarV = ref<HTMLCanvasElement[]>([])
    const canvasChartBarH = ref<HTMLCanvasElement[]>([])
    const canvasChartLine = ref<HTMLCanvasElement[]>([])
    ...
    async function exportChartToPdf(canvas: HTMLCanvasElement, filename = 'grafica.pdf') {
      ...
      pdf.text('Reporte de Ventas por Marca', pageWidth / 2, 25, { align: 'center' })

      // Obtener datos de la gráfica
      const labels = instanceChartBarV.data.labels as string[]
      const datasets = instanceChartBarV.data.datasets

      // Armar encabezados y cuerpo para la tabla
      const tableHeaders = ['', ...datasets.map((ds) => ds.label || '')]
      const tableBody = labels.map((label, i) => {
        const row = [label]
        for (const ds of datasets) {
          row.push(String(ds.data[i] ?? 0))
        }
        return row
      })
      ...
      autoTable(pdf, {
        ...
        styles: { fontSize: 10 },
        headStyles: { fillColor: [75, 75, 77] },
      })

      const finalY = (pdf as any).lastAutoTable.finalY || 80

      // Agregar gráfica debajo de la tabla con márgenes
      const chartX = 40
      const chartY = finalY + 30
      const chartWidth = pdf.internal.pageSize.getWidth() - chartX * 2
      const chartHeight = (canvasImage.height * chartWidth) / canvasImage.width
    ...
    const chartMap: Record<string, Ref<HTMLCanvasElement[]>> = {
      tabBarV: canvasChartBarV,
      tabBarH: canvasChartBarH,
      tabLine: canvasChartLine,
    }

    async function exportCurrentChart(tabValue: string) {
      const now = new Date()
      ...
      if (vtabMenuModel.value !== tabValue) {
        // Cambiar al tab seleccionado y esperar al render
        vtabMenuModel.value = tabValue
        await nextTick() // espera el DOM
        await new Promise((resolve) => setTimeout(resolve, 300)) // da tiempo al canvas de renderizar
      }

      const canvasRef = chartMap[tabValue]
      const canvas = canvasRef?.value?.[0]

      if (!(canvas instanceof HTMLCanvasElement)) {
        console.warn(`Ref para ${tabValue} no es un canvas válido`, canvas)
        return
      }

      if (!document.body.contains(canvas)) {
        console.warn('Canvas no está en el DOM aún')
        return
      }

      await exportChartToPdf(
        canvas,
        `reporte_${tabValue}_${day}_${month}_${year}_${hours}_${minutes}_${seconds}.pdf`,
      )
    }
