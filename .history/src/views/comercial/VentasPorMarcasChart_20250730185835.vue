<template>
  <v-container fluid ref="vconPrincipalRef" class="h-100">
    ...
    <v-row ref="vtabGraficaRef">
      <v-col class="my-0 py-0">
        <v-tabs v-model="vtabGraficaSelected" color="primary" class="text-medium-emphasis">
          <v-tab
      ...
      <v-col>
        <v-card elevation="5" class="pa-5 overflow-auto" :style="getCardHeight">
          ...
          <v-tabs-window v-model="vtabGraficaSelected">
            ...
            >
              <canvas
                v-if="getChartInstance(tab.chart)?.data?.datasets?.length > 0"
                :ref="tab.chart"
...
export default defineComponent({
  ...
  setup() {
    ...
    let chartInstanceV: Chart | null = null
    let chartInstanceH: Chart | null = null
    let chartInstanceLineal: Chart | null = null

    const getChartInstance = (key: string): Chart | null => {
      if (key === 'chartBarrasV') return chartInstanceV
      if (key === 'chartBarrasH') return chartInstanceH
      if (key === 'chartLineal') return chartInstanceLineal
      return null
    }
