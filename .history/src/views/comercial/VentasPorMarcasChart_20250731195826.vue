...
export default defineComponent({
  ...
  setup() {
    ...
    async function exportChartToPdf(canvasEl: HTMLCanvasElement, fileName = 'grafica.pdf') {
      const canvasImage = await html2canvas(canvasEl, {
        backgroundColor: null, // Para respetar fondo transparente
        useCORS: true,
      })
    ...
    function exportCurrentChart(tabValue: string) {
      const canvas = chartMap[tabValue]?.value
      if (canvas) {
        exportChartToPdf(canvas, `grafica_${tabValue}.pdf`)
      } else {
