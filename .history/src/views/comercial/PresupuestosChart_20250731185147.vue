...
export default defineComponent({
  ...
  setup() {
    ...
    const viewMode = ref<'year' | 'months'>('year')
    ...
    function onMonth(m: number) {
      selectedMonth.value = m
      menuOpen.value = false
      if (selectedYear.value !== undefined) {
        const mm = (m + 1).toString().padStart(2, '0')
        display.value = `${selectedYear.value}-${mm}`
      }
    }
    function onViewMode(mode: string) {
      // mantener en modo meses hasta cerrar
      if (mode === 'calendar') viewMode.value = 'months'
    }
