    ...
    */
    ...
    const fetchChartData = async () => {
      ...
      try {
        ...
        const ventas = dataset.find((d: any) => d.tipo === 'Ventas')?.TotalVentas ?? 0
        const objetivo = dataset.find((d: any) => d.tipo === 'Objetivo')?.TotalVentas ?? 0

        const real = Number(ventas)
        const obj = Number(objetivo)

        const valDesviacion = real - obj
        const valAlcance = obj !== 0 ? (real / obj) * 100 : 0
        const valDeficit = obj - real

        desviacion.value = valDesviacion.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')
        alcance.value = valAlcance.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')
        deficit.value = valDeficit.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')
