        ...
        <v-card elevation="5" class="pa-5 overflow-auto" :style="getCardHeight">
          ...
          <v-tabs-window v-model="vtabGraficaSelected">
            <v-tabs-window-item
              ...
              eager
            >
              <div v-if="chartHasData(tab.chart)">
                <canvas :ref="tab.chart" :style="getChartHeight"></canvas>
              </div>
              <div v-else class="text-center pa-5">No hay datos para mostrar en esta gráfica.</div>
            </v-tabs-window-item>
          </v-tabs-window>
        </v-card>
      </v-col>
...
export default defineComponent({
  ...
  setup() {
    ...
    // Trabajo
    const getChartInstance = (key: string): Chart | undefined => {
      return chartInstances[key]
    }

    const chartHasData = (key: string): boolean => {
  const instance = getChartInstance(key)
  return instance?.data?.datasets?.some(ds => ds.data?.length > 0) ?? false
}
