    ...
    <v-card class="rounded-lg">
      <v-form ref="formRef">
        ...
        <v-card-text class="dialog-content px-5" :style="getDialogContentPaddingTop">
          <v-row>
            <v-col cols="12" md="12">
              <div v-if="dialogPropiedades.dialog" class="position-relative">
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    watch(
      () => dialogPropiedades.dialog,
      async (isOpen) => {
        if (isOpen) {
          await nextTick() // esperar a que el canvas exista en el DOM

          if (canvasChartLine2.value) {
            if (instanceChartLine2) {
              instanceChartLine2.destroy() // evitar duplicados
            }
