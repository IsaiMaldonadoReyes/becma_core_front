          ...
          <v-card-text>
            ...
            >
              ...
              <template v-slot:item="{ props, item }">
                <v-list-item
                  v-bind="props"
                  :subtitle="item.raw.CIDCONCEPTODOCUMENTO"
                  :title="item.raw.CNOMBRECONCEPTO"
                />
              </template>
...
import { rptVentasPorConcepto } from '../../stores/modules/Comercial/rptVentasPorConcepto'
  ...
  setup() {
    ...
    const optionsChartBarV: ChartOptions = {
      ...
      plugins: {
        title: {
          display: true,
          text: 'Ventas por concepto barra',
        },
        legend: {
    ...
    const optionsChartLine: ChartOptions = {
      ...
      plugins: {
        ...
        title: {
          display: true,
          text: 'Ventas por concepto Lineal',
        },
      },
      responsive: true,
      onClick: (e, elements, chart) => {
    ...
    const modelConcepto = ref<Concepto[]>([])
    const itemsConceptos = ref<Concepto[]>([])
    ...
    const reporte = rptVentasPorConcepto()
    ...
    const fetchChartData = async () => {
      console.log('fetchChartData called with modelFechas:', modelFechas.value)
      ...
      if (isValidForm.valid) {
        ...
        const data = {
          ...
          conceptos: modelConcepto.value.map((m) => m.CIDCONCEPTODOCUMENTO).join(', '),
        ...
        try {
          ...
          let datasets = dataset.map((concepto: any) => {
            ...
            return {
              label: concepto.CNOMBRECONCEPTO,
              data: data,
              backgroundColor: color,
              borderColor: color,
              stack: 'stack_' + concepto.CNOMBRECONCEPTO, // Agrupar por stack si es necesario
            }
          })
    ...
    const fetchMarcas = async () => {
      ...
      try {
        await reporte.conceptosRpt3(data)

        itemsConceptos.value = reporte.object.data
      } catch (error) {
      } finally {
      }
    }
