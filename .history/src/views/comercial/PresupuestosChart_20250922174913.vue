  ...
  setup() {
    ...
    const optionsChartBarV: ChartOptions = {
      ...
      onClick: (e, elements, chart) => {
        if (elements.length > 0) {
          ...
          const legendParts = legend.split(' - ')
          const valorMarca = legendParts[0] // "URREA"

          const data = {
            empresa: modelEmpresa.value,
            ejercicio: modelEjercicio.value?.cejercicio,
            id_ejercicio: modelEjercicio.value?.cidejercicio,
    ...
    onMounted(() => {
      nextTick(() => {
        ...
        console.log('Canvas line Chart')
        console.log(canvasChartLine.value)
