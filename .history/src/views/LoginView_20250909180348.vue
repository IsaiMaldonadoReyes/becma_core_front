<template>
  <v-container class="fill-height login-body" fluid>
    ...
    <v-row>
              ...
              <v-tabs-window v-model="current" color="transparent">
                <v-tabs-window-item key="s1" value="s1" color="transparent">
                  ...
                  <v-row>
                    ...
                    <v-col lg="1" class="d-flex justify-end">
                  ...
                  <v-data-iterator :items="games" :items-per-page="5" :search="search">
                    <template v-slot:default="{ items }">
                      <v-container class="overflow-auto" fluid height="250px">
                        <v-row>
                          <v-col v-for="item in items" key="title" cols="auto" md="12">
                            <v-row>
                              <v-col cols="6">
                                <!--bec-autocomplete
                          ...
                          /-->
                            ...
                            <v-card
                              class="rounded d-flex justify-center align-center my-2 pa-2"
                              flat
                              min-height="60px"
                              color="#f0f0f0"
                            >
                              <v-row>
                                <v-col lg="4">Sucursal</v-col>
                                <v-col lg="4">Folio</v-col>
                                <v-col lg="3">Monto</v-col>
                                <v-col class="d-flex justify-center align-center" lg="1">
                                  <!-- vbtnEliminar -->
                                  <v-tooltip bottom color="primary" interactive>
                                    <template v-slot:activator="{ props: tooltipProps }">
                                      <v-btn
                  ...
                  <v-card-actions class="justify-end">
                    <v-btn variant="tonal" :disabled="current === 0" @click="prev"> Anterior</v-btn>

                    <v-btn color="primary" :disabled="current === steps.length - 1" @click="next">
                      Siguiente
                    </v-btn>
                  </v-card-actions>
                </v-tabs-window-item>
      ...
      <v-col lg="2">
        <v-timeline
          ...
          side="start"
          class="pa-0 ma-0"
          style="z-index: 1; height: 550px; transform: translateX(-50px)"
        >
          <v-timeline-item
          ...
          >
            ...
            <template v-slot:opposite>
              ...
              <!--v-chip :color="itemColor(i)" size="small" variant="flat">
                    {{ i < current ? 'Completado' : i === current ? 'En curso' : 'Pendiente' }}
                  </v-chip-->
            </template>
          </v-timeline-item>
        </v-timeline>
    ...
    <!--v-card color="transparent" elevation="0" width="70%" height="500px">
      <v-row>
        <v-col cols="12" lg="9"> </v-col>
        <v-col cols="12" lg="3" class="d-flex justify-start pa-0 ma-0"> </v-col>
      </v-row>
    </v-card-->
    <dialog-information
...
<script lang="ts">
import { defineComponent, ref, shallowRef, mergeProps, computed } from 'vue'
...
export default defineComponent({
  ...
  setup() {
    ...
    const steps: Step[] = [
      {
        value: 's1',
        title: '1. Ticket',
        subtitle: 'Ingresa información correspondiente al ticket',
        icon: 'mdi-invoice',
      },
      {
        value: 's2',
    ...
    // 4) Lottie por paso: cambia automáticamente según `current`
    const animationMap: Record<string, string> = {
      s1: '/src/assets/images/doctor4.json',
      s2: '/src/assets/images/doctor2.json',
      s3: '/src/assets/images/doctor3.json',
    }
    const currentAnimation = computed(() => animationMap[current.value] ?? animationMap.s1)
