  ...
  <v-app>
    <v-app-bar app clipped-left height="60" color="#E6E9F5">
      <v-app-bar-nav-icon
        ...
        @click.stop="drawerParent = !drawerParent"
      />
      <v-toolbar-title>
...
export default defineComponent({
  ...
  setup() {
    ...
    const drawerChilden = ref(true)

    const onClicDrawer = () => {
      if (drawerParent) {
        drawerChilden.value = true
      } else {
        drawerChilden.value = false
      }
    }

    return { toggleTheme, drawerParent, railParent, drawerChilden, railChildren }
  },
})
</script>
<style>
