<template>
  <v-menu :close-on-content-click="false">
    <template v-slot:activator="{ props }">
      ...
      <v-avatar class="mx-2" v-bind="props">
        <v-img v-if="item.imagen" alt="DD" :src="item.imagen" />
        <span class="text-h5" v-else>{{ getIniciales }}</span>
      </v-avatar>
      <v-avatar class="mx-2" v-else color="primary" v-bind="props"> </v-avatar>
    </template>
        ...
        <v-list-item lines="three">
          <template v-slot:prepend>
            <v-avatar size="x-large" v-if="item.imagen">
              <v-img alt="DD" :src="item.imagen" />
            </v-avatar>
            <v-avatar v-else color="primary" size="large">
              <span class="text-h5">{{ getIniciales }}</span>
            </v-avatar>
          </template>
