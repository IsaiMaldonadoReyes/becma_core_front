<template>
  <div>
    <v-dialog v-model="dialogPropiedades.dialog" width="auto" persistent>
      <v-card>
        <v-form ref="form" v-model="isValid">
          ...
          <v-card-text class="dialog-content px-5" :style="getDialogContentPaddingTop">
            <v-avatar size="20%" @click="show = true">
              <v-img
                :src="
                  propsAvatar.data ? propsAvatar.data : 'https://cdn.vuetifyjs.com/images/john.jpg'
                "
                alt="DD"
                draggable
              />
      ...
      >
        <template #cancel>
          <v-btn color="primary" @click="onCloseAvatar">Cancelar</v-btn>
        </template>
        <template #closeIcon>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const handleSuccess = (file: File, close: Function) => {
      // upload flow
      close() // close pop-up

      propsAvatar.value = {
        ...propsAvatar.value, // Mantiene el resto de las propiedades
        data: file, // Asigna el nuevo archivo
      }
    }
