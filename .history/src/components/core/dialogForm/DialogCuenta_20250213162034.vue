    ...
    <v-dialog v-model="dialogPropiedades.dialog" width="auto" persistent>
      <v-card>
        <v-form ref="form" v-model="isValid">
          <v-card-title
            class="position-fixed bg-surface-light dialog-header border-b-0"
            ref="dialogHeader"
          >
            <v-row>
          ...
          <v-card-text class="dialog-content px-5" :style="getDialogContentPaddingTop">
            <v-row>
              <v-col cols="6">
                <v-card
                  class="bg-surface-light pa-3 d-flex justify-center align-center"
                  elevation="0"
                >
                  <v-card-text>
                    <v-avatar size="50%" @click="show = true">
                      <v-img
                        :src="
                          propsAvatar.avatar
                            ? propsAvatar.avatar
  ...
  setup(props, { emit }) {
    ...
    const handleSuccess = (file: File) => {
      /**file.arrayBuffer().then((file) => {
        ...
        propsAvatar.value.avatar = base64Img
      })**/
